...
import fi.aalto.hoaxify.configuration.AppConfiguration;
import fi.aalto.hoaxify.error.ApiError;
import fi.aalto.hoaxify.file.FileAttachment;
import fi.aalto.hoaxify.file.FileAttachmentRepository;
import fi.aalto.hoaxify.file.FileService;
import fi.aalto.hoaxify.hoax.Hoax;
import fi.aalto.hoaxify.hoax.HoaxRepository;
import fi.aalto.hoaxify.hoax.HoaxService;
import fi.aalto.hoaxify.hoax.vm.HoaxVM;
import fi.aalto.hoaxify.shared.GenericResponse;
import fi.aalto.hoaxify.user.User;
import fi.aalto.hoaxify.user.UserRepository;
import fi.aalto.hoaxify.user.UserService;
...
public class HoaxControllerTest {
	...
	public void postHoax_whenHoaxIsValidAndUserIsAuthorized_hoaxCanBeAccessedFromUserEntity() {
		...
		Hoax hoax = TestUtil.createValidHoax();
		postHoax(hoax, Object.class);
		/* Use EntityManager to cope with the exception caused by Hibernate lazy initialization */
	...
	public void postHoax_whenHoaxIsValidAndUserIsAuthorized_receiveHoaxVM() {
		...
		Assertions.assertThat(response.getBody().getUser().getUsername()).isEqualTo("user1");
	}
	...
	public void postHoax_whenHoaxHasFileAttachmentAndUserIsAuthorized_hoaxFileAttachmentRelationIsUpdatedInDatabase() throws IOException {
		...
		Assertions.assertThat(inDB.getAttachment().getId()).isEqualTo(savedFile.getId());
	}
	...
	public void getHoaxes_whenThereAreHoaxes_receivePageWithHoaxVM() {
		...
		Assertions.assertThat(storedHoax.getUser().getUsername()).isEqualTo("user1");
	}
	...
	public void getHoaxesOfUser_whenMultipleUserExistWithMultipleHoaxes_receivePageWithMatchingHoaxesCount() {
		/* Create first user with three Hoaxes */
		User userWithThreeHoaxes = userService.save(TestUtil.createValidUser("user1"));
		IntStream.rangeClosed(1, 3).forEach(i -> hoaxService.save(userWithThreeHoaxes, TestUtil.createValidHoax()));

		/* Create second user with five Hoaxes */
		User userWithFiveHoaxes = userService.save(TestUtil.createValidUser("user2"));
		IntStream.rangeClosed(1, 5).forEach(i -> hoaxService.save(userWithFiveHoaxes, TestUtil.createValidHoax()));
	...
	public void getNewHoaxes_whenThereAreHoaxes_receiveListOfItemsAfterProvidedId() {
		...
		/* Since all the new Hoaxes will be returned, therefore List return type should be used */
